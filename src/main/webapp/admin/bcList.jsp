    ...
    <script type=text/javascript>
        ...
        function  changePageA(obj) {
            with (document.getElementById("form1")){
                method = "post";
                action = "<%=path%>/bcList.action?index="+obj+"&subjectInfo=${subjectInfo}";
                submit();
            }
        }
        ...
        function changePage(obj){
            ...
            with(document.getElementById("form1")){
                method = "post";
                action = "<%=path%>/bcList.action?index="+obj.value+"&subjectInfo=${subjectInfo}";
                submit();
            }
        }
        ...
        function modify() {
            ...
            if (count == 1) {
                with (document.getElementById("form1")) {
                    method = "post";
                    action = "<%=path %>/bcMb.action?id=" + j+"&subjectInfo=${subjectInfo}";
                    submit();
                }
            }
        }
        ...
        function deletes() {
            ...
            if (window.confirm("确认删除吗？")) {
                with (document.getElementById("form1")) {
                    method = "post";
                    action = "<%=path %>/bcDel.action?idList=" + arrayObj;
                    submit();
                }
            }
        }
        ...
        <TR>
            ...
            <TD vAlign=top width="100%" bgColor=#ffffff>
                <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
                    <TR>
                        <TD class=manageHead>当前位置：管理首页 &gt; 题库 &gt; 计算题管理</TD>
                    </TR>
                    <TR>
                        <TD height=2></TD>
                    </TR>
                            ...
                            <table id=grid
                                ...
                                <c:forEach items="${page.data}" var="bean">
                                    <tr>
                                        <td>${bean.title}</td>
                                        <td>${bean.da}</td>
                                        <td><input name="selectFlag" type="checkbox" id="${bean.id}"> </td>
                                    </tr>
                                </c:forEach>
