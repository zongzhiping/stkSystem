  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt; 阅卷 &gt; 半自动阅卷列表</TD></TR>
        <TR>
        ...
        <TR>
          <TD>
			...
			for(int i = 0;i<list.size();i++){
				StuKs bean = (StuKs)list.get(i);
				int stuid= bean.getStuid();
				int kid=bean.getKid();
				String stukid=stuid+","+kid;
			%>
              <TR 
              style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
                <TD><%=bean.getUserNo() %></TD>
                <TD><%=bean.getRealName() %></TD>
                <TD><INPUT id="selectFlag" 
                  type="checkbox"  name="selectFlag"  value="<%=stukid%>">
</TD></TR>
<%} %>
        ...
        <TR>
          <TD align=right height=25><INPUT id=boxListValue type=hidden 
            name=boxListValue> <INPUT class=button id=button1  value="阅卷" name=button1 onclick="modify()"> 
             </TD></TR>
       </TBODY></TABLE></TD>
