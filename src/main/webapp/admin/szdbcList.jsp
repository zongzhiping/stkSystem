...
function modify() {
    ...
    var id = document.getElementsByName("selectFlag")[j].value;
        window.open ('kstAdd.action?id='+id+'&type=4&kid=<%=kid%>');
    }
}
...
function deletes() {
        with (document.getElementById("form1")) {
            method = "post";
            action = "<%=path %>/wcks2.action?id=<%=kid%>&type=1";
            submit();
        }
}
  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt; 题库 &gt; 计算题列表</TD></TR>
        <TR>
...
<INPUT class=button id=button2  value="完成" type=button name=button2 onclick="deletes()"> 
           </TD></TR>
       </TBODY></TABLE></TD>
