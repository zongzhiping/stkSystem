...
function modify() {
    ...
    var id = document.getElementsByName("selectFlag")[j].value;
        window.open ('kstAdd.action?id='+id+'&type=2&kid=<%=kid%>');
    }
}
...
function deletes() {
    if (window.confirm("确认进入吗？")) {
        with (document.getElementById("form1")) {
            method = "post";
            action = "<%=path %>/szdpdList.action?id=<%=kid%>";
            submit();
        }
    }
}
  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt; 题库 &gt; 随机制定 多选题列表</TD></TR>
        <TR>
...
<INPUT class=button id=button2  value="进入 判断题" type=button name=button2 onclick="deletes()"> 
           </TD></TR>
       </TBODY></TABLE></TD>
